...
@Composable
fun EditProductScreen(navController: NavHostController, menuId: String) {
    ...
    Log.e("Product Edit Screen1", "menuListState: $menuListState")




    val branch_id = remember {mutableStateOf("")}
    val menu_type = remember {mutableStateOf("daily_stock")}
    val menu_name = remember {mutableStateOf("")}
    val menu_price = remember {mutableStateOf("")}
    val menu_description = remember {mutableStateOf("")}
    val menu_qty = remember {mutableStateOf("")}
        ...
        Column(modifier = Modifier.padding(16.dp),verticalArrangement = Arrangement.Top) {
            ...
            when {
                menuListState.isEmpty() -> {
                    ...
                    if (base64Text.value != "") {
                        val encodedImage = base64Text.value
                        val cleanMenuId = menuId.replace("[", "").replace("]", "")
                        Button(onClick = {
                            if (storedToken != null ) {
                                menuViewModel.updateMenuImage(storedToken, cleanMenuId, encodedImage)
                                Log.e("FoodScreen", "order process in viewmodel")


                                scope.launch {
                                    delay(1000L)
                                    Log.e("Edit Image Screen", "respon: ${menuViewModel.updateImageResult.value}")
...
//                    Log.e("Edit Product Screen", "menuListState ${menuListState[0]}")
